			...
			<c:forEach items="${list}" var="act">
				...
				<tr>
					...
					<td>
						...
						<c:if test="${not empty task.assignee}">
							<a href="#" onclick="openDialogView('工单审批','${ctx}/act/task/form?taskId=${task.id}&taskName=${fns:urlEncode(task.name)}&taskDefKey=${task.taskDefinitionKey}&procInsId=${task.processInstanceId}&procDefId=${task.processDefinitionId}&status=${status}','70%','60%')">任务办理</a>
						</c:if>
